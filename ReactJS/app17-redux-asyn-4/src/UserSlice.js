...
export const handleAddUserAsync=createAsyncThunk(
    "users/handleAddUserAsync",async(user)=>{
        const response=fetch("http://localhost:3000/users3",{
            method: "POST",
            headers: {
...
export const deleteUserAsync = createAsyncThunk(
    ...
    async (user) => {
      const response = fetch("http://localhost:3000/users3/" + user.id, {
        method: "DELETE",
      });
      const dd = await (await response).json();
      const resp = await fetch("http://localhost:3000/users3");
      const data = await resp.json();
      return data;
    }
  );
